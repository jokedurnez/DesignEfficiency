...
class GeneticAlgorithm(object):
    ...
    def __init__(self,ITI,TR,L,P,C,rho,weights,Aoptimality=True,saturation=True,resolution=0.1,G=20,q=0.01,I=4,cycles=1000,preruncycles=500,ConfoundOrder=3,MaxRepeat=6):
        self.ITI = ITI
        self.TR = TR
        self.L = L
        self.P = P
    ...
    def GeneticAlgorithmGeneration(self,Generation):
        ...
        for ord in range(len(Generation['order'])):
            RepCheck = ''.join(str(e) for e in [0]*maxrepeat) in ''.join(str(e) for e in Generation['order'][ord])
            if RepCheck:
