...
class Design(object):
    ...
    def __init__(self,ITI,TR,L,P,C,rho,weights,Aoptimality=True,saturation=True,resolution=0.1,G=20,q=0.01,I=4,cycles=1000):
        self.ITI = ITI
        self.TR = TR
        self.L = L
        self.P = P
    ...
    def GeneticAlgoritm(self):
        ...
        for gen in range(self.cycles):
            ...
            NextGen = self.GeneticAlgoritmGeneration(Generation)
            Generation = NextGen["NextGen"]
            Best.append(NextGen['FBest'])

        OptInd = np.arange(len(Generation['F']))[Generation['F']==np.max(Generation['F'])]
        out = {
    ...
    def GeneticAlgoritmGeneration(self,Generation):
        Generation = self.GeneticAlgoritmCrossover(Generation)
        Generation = self.GeneticAlgoritmMutation(Generation)
        Generation = self.GeneticAlgoritmImmigration(Generation)
    ...
    def FcCalc(self,Design):
        Q = np.zeros([self.rc,self.rc,10])
        for n in range(self.L):
            for r in range(10):
                if n>(r-1):
                    Q[Design['order'][n],Design['order'][n-r],r] += 1
        Qexp = np.zeros([self.rc,self.rc,10])
